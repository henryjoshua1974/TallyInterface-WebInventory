...
function TransferData() {
      ...
      if (PropertyOptions.length > 0) {
        PropertyOptions.forEach((item) => {
          if (item.propertyCode == selectedProperty) {
            xmlFileName =
              "TallyTransferInventory" +
              item.propertyName.replace(/\s+/g, "") +
              ".xml";
          }
        });
      }
  ...
  const loadAllPropertyDetailsToState = (val) => {
    // setAllPropertyList((prev) => [...prev, val]);
  };
  ...
  const selectAllCheckboxes = () => {
    ...
    PropertyOptions.map((item) => (
      setSelectedProperty((prev) => [...prev, item.propertyCode])
    ));
   


    // setSelectedProperty([...PropertyOptions]);
        ...
        <form className="FormDiv" onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            <div className="propertylistclass">
              ...
              <div>
              <button type="button" className="btn btn-light" onClick={selectAllCheckboxes}>Select All</button>
              &nbsp;&nbsp;&nbsp;
              <button type="button" className="btn btn-light" onClick={deselectAllCheckboxes}>Deselect All</button>
            ...
            <div className="classlistbox">
              {PropertyOptions.map((item) => (
                <div key={item.propertyCode}>
                  {loadAllPropertyDetailsToState(item.propertyCode)}
                  <label>
                    <input
                      type="checkbox"
          ...
          {execTransferDataFlag && (
            <label className="ErrorTextClass">{errorText}</label>
          )}
          {showMessageTextFlag && (
            <label className="MessageTextClass">{messageText}</label>
          )}
        </form>
      </div>
    </div>
